...
- **Roles (RBAC)** — 10 granular permission bits, built‑in `USER`/`SUDO`/`ADMIN`/`OWNER` plus
  custom roles. You can never grant a permission you don't hold yourself; the admin UI only
  shows buttons your role allows. Telegram IDs listed in `SUDO_IDS` receive the built‑in
  `SUDO` role on `/start` — full control below `OWNER`, and protected from other admins
  (they can't be blocked or reassigned).
- **Telegram log group** — every audited action (purchases, top‑ups, admin actions, errors)
  can be mirrored into a Telegram group: set `LOG_GROUP_ID` (optionally filter event classes
  with `LOG_GROUP_EVENTS`).
...
| `LOG_GROUP_ID`                            | Telegram chat/group that mirrors every audit entry            | –                                 |
| `LOG_GROUP_EVENTS`                        | Event classes sent to the group (`all`, `payments`, `purchases`, `admin`, `security`, `errors`, `off`) | `all` |
| `SUDO_IDS`                                | Comma-separated Telegram IDs that get the built-in `SUDO` role | –                                |
