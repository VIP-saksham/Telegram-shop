...
async def role_view_handler(call: CallbackQuery):
    ...
    if not role['default'] and role['name'] not in ('USER', 'SUDO', 'ADMIN', 'OWNER') and user_count == 0:
...
async def assign_role_list(call: CallbackQuery):
    ...
    if target_role_name == 'OWNER' or EnvKeys.is_sudo(target_id):
        await call.answer(localize('admin.users.cannot_touch_sudo'), show_alert=True)
        return
