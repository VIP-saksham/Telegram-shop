...
def _is_sudo_target(user_id: int) -> bool:
    """Sudo users (and the env OWNER) are protected from other admins' actions."""
...
async def block_user_handler(call: CallbackQuery):
    ...
    if role_name == 'OWNER' or _is_sudo_target(user_id):
        await call.answer(localize('admin.users.cannot_block_sudo'), show_alert=True)
        return
