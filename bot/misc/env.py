...
class EnvKeys(ABC):
    ...
    # Telegram log group — every audited action is mirrored into this chat.
    LOG_GROUP_ID: Final = _get_optional("LOG_GROUP_ID", "")
    ...
    SUDO_IDS: Final = _get_optional("SUDO_IDS", "")
    ...
    # Parsed SUDO ids (deduplicated, order-preserving).
    SUDO_ID_LIST: Final = tuple(
        dict.fromkeys(
            int(x) for x in SUDO_IDS.replace(" ", "").split(",")
    ...
    def is_sudo(user_id: int) -> bool:
        return user_id == EnvKeys.OWNER_ID or user_id in EnvKeys.SUDO_ID_LIST
